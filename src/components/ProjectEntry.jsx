...
export default function ProjectEntry({ project, compact = false }) {
  return <article className={`project-entry ${compact ? 'project-entry--compact' : ''}`}>
    <div className="project-heading"><span className="project-number">{project.number}</span><div><p className="eyebrow">{project.category}</p><h2>{project.title}</h2></div><span className="project-year">{project.year}</span></div>
